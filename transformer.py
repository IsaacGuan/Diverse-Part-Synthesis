...
def transform(x_obj, w_scale, w_trans):
    # removes the last dimension
    if len(x_obj.shape) == 5:
        obj = x_obj[...,0]
    else:
        obj = x_obj
    ...
    T_xy = s_xy + t_xy
    transformed_xy = spatial_transformer_network(obj, T_xy)

    # reshape the obj so that it starts with z dim: [batch, x, z, y]
    transposed_transformed_xy = tf.transpose(transformed_xy, [0,2,3,1])

    # transform the z dim 
    s_zx = tf.matmul(w_scale, tf.constant([[1,0,0,0,0,0]],dtype=tf.float32)) + tf.matmul(tf.ones([x_obj.shape[0], 1]), tf.constant([[0,0,0,0,1,0]],dtype=tf.float32))
    t_zx = tf.matmul(w_trans_z, tf.constant([[0,0,1,0,0,0]],dtype=tf.float32))
    T_zx = s_zx + t_zx
    transformed_zxy = spatial_transformer_network(transposed_transformed_xy, T_zx)
